...
As mentioned in part one of this series, the nice thing about django, and python in general is its extensibility and flexibility to address future requirements as they pop up.  
...
```
You'll see the tables we've created for ACI and PgAdmin, and if you've followed along with previous updates, the 30+ tables created for Meraki, SDWAN, DCNM, DNAC, OpenVuln API.
...
With the call home email alert functionality of ACI and other Cisco products, do we really need to? No, but worst case, maybe the functions would be useful for another project?  Or if we wanted to include some other devices in our dashboard that don't support call home.
...
Until now, we've been calling our API functions arbitrarily from our scripts, this being the last update, we'll consolidate these into a single script via a While Loop so they can be aggregated and run continuously.
